...
def freeling_oldSp(path):
    ...
    if not os.path.exists(os.environ["FREELINGDIR"]+"/share/freeling") :
       ...
       # Error message specific to the use of Freeling here
    if not os.path.exists(path+"/txt") :
       print("Missing 'txt' directory in "+path+". The specified path should contain a 'txt' folder containing sources in .txt",
             file=sys.stderr)
       sys.exit(1)


    # Create xml dir
    os.makedirs(path + '/xml', exist_ok=True)
    ...
    for f in files:
        ...
        while (lin) :
            ...
            for s in ls :
                ws = s.get_words();
                with open(path + '/xml/' + f.split(".")[0].split("/")[-1] + ".xml", 'w') as f_out:
                    newxml = "<ab>"
                    for w in ws :
